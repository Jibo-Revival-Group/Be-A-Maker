...
window.onload = function () {
   ...
   /* document.getElementById('projectLoadButton').onclick = function () {
    ...
    vm.on('SCRIPT_GLOW_ON', data => {
        ...
        try {
            if (window.jibo.browser === "ios") {
                window.webkit.messageHandlers.startScript.postMessage(commandJson);
            }else if(window.jibo.browser === "android"){
                window.appInterface.startScript(JSON.stringify(commandJson));
            }
        } catch (err) {
    ...
    vm.on('SCRIPT_GLOW_OFF', data => {
        ...
        try {
            if (window.jibo.browser === "ios") {
                window.webkit.messageHandlers.finishScript.postMessage(commandJson);
            }else if(window.jibo.browser === "android"){
                window.appInterface.finishScript(JSON.stringify(commandJson));
            }
        } catch (err) {
            // document.getElementById("debug").innerHTML =err+" ";
    ...
    if (window.jibo.browser === "ios") {
        webkit.messageHandlers.scratchLoad.postMessage(commandJson);
    }else if(window.jibo.browser === "android"){
        window.appInterface.scratchLoaded(JSON.stringify(commandJson));
    }
    // populateMedia();

    window.greenFlag = () => {vm.greenFlag();};
    window.stopAll = () => {vm.stopAll();};
...
window.toXml =  () => {
    ...
    if (window.jibo.browser === "ios") {
        webkit.messageHandlers.comandHandler.postMessage({
                                                     action: 'Save',
                                                     arg: xml_text
                                                     });
    }else if(window.jibo.browser === "android"){
        window.appInterface.commandHandler(JSON.stringify({
                                                     action: 'Save',
                                                     arg: xml_text
                                                     }));
    }
